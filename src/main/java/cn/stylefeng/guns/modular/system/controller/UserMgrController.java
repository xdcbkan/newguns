...
import cn.stylefeng.guns.modular.system.mapstruct.UserConverter;
...
public class UserMgrController extends BaseController {
     ...
     */
    @Permission
    @RequestMapping("/articleTypeAssign/{userId}")
    public String articleTypeAssign(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        User user = this.userService.selectOne(new EntityWrapper<User>().eq("id", userId));
        model.addAttribute("userId", userId);
        model.addAttribute("userAccount", user.getAccount());
        return PREFIX + "user_articletypeassign.html";
    }
     ...
     */
    @Permission
    @RequestMapping("/projectTypeAssign/{userId}")
    public String projectTypeAssign(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        User user = this.userService.selectOne(new EntityWrapper<User>().eq("id", userId));
        model.addAttribute("userId", userId);
        model.addAttribute("userAccount", user.getAccount());
        return PREFIX + "user_projecttypeassign.html";
    }
...
//        assertAuth(userId);
     ...
     * 跳转到管理员修改用户密码界面
     */
    @RequestMapping("/openModifyPwd/{userId}")
    public String openModifyPwd(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        model.addAttribute("userId", userId);
        return PREFIX + "user_modifypwd.html";
    }
    ...
    @ResponseBody
    public Object modifyPwd(@RequestParam Integer userId, @RequestParam String newPwd, @RequestParam String rePwd) {
        if (!newPwd.equals(rePwd)) {
            throw new ServiceException(BizExceptionEnum.TWO_PWD_NOT_MATCH);
        }
        User user = userService.selectById(userId);
...
//            assertAuth(user.getId());
            ...
            if (shiroUser.getId().equals(user.getId())) {
                HistoryUserinfo historyUserinfo = UserConverter.INSTANCE.userToHistory(oldUser);
                historyUserinfo.setCreatetime(now());
...
//        assertAuth(userId);
        ...
        System.out.println("roleIdArray: " + roleIdArray);
        // 查询是否包含要设置文章领取上限的角色 的 领取数值
        Integer receivenum = roleService.selectRoleReceiveNum(roleIdArray);
        if (ToolUtil.isNotEmpty(receivenum)) {
            EntityWrapper<User> ew = new EntityWrapper<>();
            ew.eq("id", userId);
            String setSql = "receivenum = " + receivenum + ",residuereceivenum = " + receivenum;
     ...
     * 分配文章分类
     */
    @RequestMapping("/setArticleType")
//    @BussinessLog(value = "分配角色", key = "userId,roleIds", dict = UserDict.class)
    @Permission
    @ResponseBody
    public ResponseData setArticleType(@RequestParam("userId") Integer userId, @RequestParam("articleTypeIds") String articleTypeIds) {
        if (ToolUtil.isOneEmpty(userId, articleTypeIds)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
...
//        assertAuth(userId);
        this.userService.setArticleTypes(userId, articleTypeIds);
        return SUCCESS_TIP;
    }
    ...
    @Permission
    @ResponseBody
    public ResponseData setProjectType(@RequestParam("userId") Integer userId, @RequestParam("projectTypeIds") String projectTypeIds) {
        if (ToolUtil.isOneEmpty(userId, projectTypeIds)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
...
//        assertAuth(userId);
        this.userService.setProjectTypes(userId, projectTypeIds);
        return SUCCESS_TIP;
    }
