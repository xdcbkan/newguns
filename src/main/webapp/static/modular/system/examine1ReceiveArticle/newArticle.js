...
NewArticle.openNewArticleDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/newArticle/newArticle_update/' + NewArticle.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 打开查看文章页面效果
 */
NewArticle.openNewArticleView = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '文章详情',
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/newArticle/newArticle_view/' + NewArticle.seItem.id
        });
        this.layerIndex = index;
    }
};
...
NewArticle.toEdit = function () {
    ...
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/newArticle/newArticle_update/' + articleId
    });
    this.layerIndex = index;
};
